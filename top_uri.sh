...
# =================================================
# top_uri - Monitor de URIs en tiempo real
# =================================================
# Muestra un histograma vivo de las URIs
...
TEMP_FILE=$(mktemp /tmp/nginx_uri_XXXXXX.tmp)
REFRESH_INTERVAL=1
TOP_N=30
...
show_histogram() {
    ...
    output+=$(echo -e "${GREEN}=== Top URIs en tiempo real ===${NC}")
    output+=$'\n'
...
sudo tail -f "$LOG_FILE" | jq --unbuffered -r '.uri' >> "$TEMP_FILE" &
TAIL_PID=$!
