...
# =================================================
# top_ip - Monitor de IPs en tiempo real
# =================================================
# Muestra un histograma vivo de las IPs
...
TEMP_FILE=$(mktemp /tmp/nginx_ips_XXXXXX.tmp)
REFRESH_INTERVAL=1
TOP_N=30
...
show_histogram() {
    ...
    output+=$(echo -e "${GREEN}=== Top IPs en tiempo real ===${NC}")
    output+=$'\n'
...
sudo tail -f "$LOG_FILE" | jq --unbuffered -r '.ip' >> "$TEMP_FILE" &
TAIL_PID=$!
